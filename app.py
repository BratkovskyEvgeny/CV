...
def detect_objects(our_image):
    ...
    if len(indexes)>1:
        st.success("Найдено {} объекта следующих классов - {}".format(len(indexes),[item for item in set(items)]))
    else:
        st.success("Найдено {} объекта следующих классов - {}".format(len(indexes),[item for item in set(items)]))
...
def object_main():
    ...
    choice = st.radio("options",("Пример для демонстрации", "Выбрать изображение из коллекции"), label_visibility='collapsed')
    st.write()
