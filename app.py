...
def object_main():
    ...
    st.sidebar.info("Веб-приложение для демонстрации алгоритма детекции объектов (проект для базового потока Deep Learning School). Создано Братковским Евгением Викторовичем (User ID (Stepik.org): 528271325).")
    st.title("Обнаружение объектов")
    st.write("<h6 style='text-align: center;'>YOLO или You Only Look Once — это популярная архитектура нейронных сетей, которая используется для детекции (распознавания) объектов на изображениях. В проекте использована YOLOv3. </h6>", unsafe_allow_html = True)
